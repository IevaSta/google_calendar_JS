export function renderEvent(event, activeDay, id, onEventDelete) {
    ...
    const sundayDate = new Date(activeDayDate.setDate(activeDay.getDate() - activeDay.getDay()));
    //converting time to pixels
    const eventTop = timeConverterToPixels(event.start).toString();
    const eventLength = (timeConverterToPixels(event.end) - timeConverterToPixels(event.start)).toString();
    ...
    const selectStartEndDOM = createDOM('span', 'event-time', `${event.selectBusyFree === 'BUSYYY' ? 'Busy' : 'Free'}`);
    ...
    columnList.forEach((_, i) => {
        const day = new Date(sundayDate.getFullYear(), sundayDate.getMonth(), sundayDate.getDate() + i);
...
function timeConverterToPixels(time) {
    ...
    if (hoursNumber)
        hoursNumber = hoursNumber * cellHeightInPixels;
    if (minNumber)
        minNumber = minNumber * minHeightInPixels;
